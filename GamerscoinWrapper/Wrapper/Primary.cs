...
namespace GamerscoinWrapper.Wrapper
{
    public sealed class Primary : IBaseConnector
    {
        ...
        public Primary()
        {
            ...
            {
                throw new ArgumentException("You have to add a Gamerscoin qt username setting with key: Username");
            }

            if (String.IsNullOrWhiteSpace(_primarypassword))
            {
                throw new ArgumentException("You have to add a Gamerscoin qt password setting with key: Password");
            }
        }
